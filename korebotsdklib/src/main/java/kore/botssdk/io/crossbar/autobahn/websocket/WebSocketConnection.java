...
import kore.botssdk.io.crossbar.autobahn.websocket.messages.CannotConnect;
...
import kore.botssdk.io.crossbar.autobahn.websocket.messages.Quit;
import kore.botssdk.io.crossbar.autobahn.websocket.messages.RawTextMessage;
import kore.botssdk.io.crossbar.autobahn.websocket.messages.ServerError;
import kore.botssdk.io.crossbar.autobahn.websocket.messages.ServerHandshake;
...
public class WebSocketConnection implements IWebSocket {
    ...
    static final IABLogger LOGGER = ABLogger.getLogger(WebSocketConnection.class.getName());

    private Handler mMasterHandler;

    WebSocketReader mReader;
    WebSocketWriter mWriter;
    ...
    String mWsPath;
    String mWsQuery;
    String[] mWsSubprotocols;
    Map<String, String> mWsHeaders;

    IWebSocketConnectionHandler mWsHandler;

    WebSocketOptions mOptions;

    boolean mActive;
    boolean mPrevConnected;
    boolean onCloseCalled;

    ScheduledExecutorService mExecutor;
    ScheduledFuture<?> mPingerTask;

    final Runnable mAutoPinger = new Runnable() {
        @Override
        public void run() {
            if (mReader != null &&
     ...
     */
    class WebSocketConnector extends Thread {
    ...
    void closeReaderThread(boolean waitForQuit) {
        if (mReader != null) {
            mReader.quit();
            if (waitForQuit) {
                try {
    ...
    void failConnection(int code, String reason) {
    ...
    <T> T getOrDefault(Map<?, ?> obj, Object key, T default_value) {
     ...
     * @param protocols
     */
    void setEnabledProtocolsOnSSLSocket(Socket socket, String[] protocols) {
